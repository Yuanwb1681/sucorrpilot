...
int read_su_trace(FILE *fp, segy *tr){
    size_t header_count=0;
    size_t data_count=0;

    if(fp==NULL||tr==NULL) return -1;
    
    header_count =fread(tr, 1, HDRBYTES, fp);

    if(header_count==0){
        if(feof(fp)) return 0;
        return -1;
    }
    if(header_count!=HDRBYTES) return -1;
    if(tr->ns==0 ||tr->ns>SU_NFLTS) return -1;
    
    data_count=fread(tr->data, sizeof(float), tr->ns, fp);
    if(data_count!=tr->ns) return -1;

    return 1;
}

int write_su_trace(FILE *fp, const segy *tr){
    size_t header_count=0;
    size_t data_count=0;

    if(fp==NULL||tr==NULL) return 0;
    if(tr->ns==0||tr->ns>SU_NFLTS) return 0;
    header_count=fwrite(tr, 1, HDRBYTES, fp);
    if(header_count!=HDRBYTES) return 0;

    data_count=fwrite(tr->data, sizeof(float), tr->ns, fp);

    if(data_count!=tr->ns) return 0;
    return 1;
}
