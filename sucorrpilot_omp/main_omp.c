...
char *sdoc[] = {
    "SUCORRPILOT_OMP - 批量完成 SU 数据与控制信号的互相关",
    "",
    "用法：",
    "  sucorrpilot_omp input_dir=raw_su output_dir=corr_su ",
    "              pilot_dir=control_signal outtime=4.0 nthreads=max-2 verbose=1",
    "",
...
int main(int argc,char **argv){
    ...
    if(!getparint("nthreads", &nthreads)) {
    ...
    if(nthreads<=0) err("nthreads must be greater than 0");
    ...
    if(verbose){
        ...
        warn("OpenMp threads = %d",nthreads);
    }
    ...
    pilot_read_status=read_su_trace(fp_pilot, &pilot);
    if(pilot_read_status!=1){
        fclose(fp_pilot);
        fp_pilot=NULL;
        globfree(&pilot_list);
        if(pilot_read_status==0)
            err("pilot file is empty:%s",pilot_name);
        else
    ...
    #pragma omp parallel for schedule(dynamic,1) reduction(+:total_trace_count)
    for (i=0; i<total_files; i++) {
        ...
        int file_trace_count=0;
        int trace_read_status;

        char out_path_local[1024];
        ...
        if(fp_in==NULL)
            err("cannot open input file %s",input_path);

        fp_out=fopen(out_path_local, "wb");
        if (fp_out==NULL){
            fclose(fp_in);
            err("cannot create output file %s",out_path_local);
        }

        file_trace_count=0;
        while ((trace_read_status=read_su_trace(fp_in, &tr))==1) {
            ...
            if(!corr_initialized){
                ...
                if(verbose>=2){
                    #pragma omp critical(log_output)    
                    {
                        warn("thread %d initialized FFT for %s",omp_get_thread_num(), input_name);
            ...
            if(!write_su_trace(fp_out, &out)){
                fclose(fp_in);
                fclose(fp_out);

                free_corr_workspace(&corr_workspace);
                err("failed to write output trace:file=%s trace=%d", input_name, file_trace_count+1);
            }
        ...
        if(trace_read_status < 0){
            fclose(fp_in);
            fclose(fp_out);

            free_corr_workspace(&corr_workspace);

            err("invalid or incomplete SU trace: file=%s trace=%d", input_name, file_trace_count + 1);
        }
        ...
        if(verbose>=2){
            #pragma omp critical(log_output)
            {
            warn("thread %d :[%lu/%lu] completed %s: %d traces", omp_get_thread_num(), (unsigned long)(i + 1), 
                    (unsigned long)total_files, input_name, file_trace_count);
            }
        }
    }
